...
describe("table markdown round trip", () => {
  ...
  it("preserves multiple inline math spans with LaTeX escapes in the same cell", () => {
    const cells = getCellTexts(
      [
        "| Condition | Facts |",
        "| --- | --- |",
        "| Absolute time difference | The system checks $\\left|Clock_{NTP_{Camera1}} - Clock_{GPS_{Camera1}}\\right|$ and $\\left|Clock_{NTP_{Camera2}} - Clock_{GPS_{Camera2}}\\right|$. |",
        "",
      ].join("\n")
    );
    ...
    expect(cells[1][0]).toBe("Absolute time difference");
    expect(cells[1][1]).toBe(
      "The system checks \\left|Clock_{NTP_{Camera1}} - Clock_{GPS_{Camera1}}\\right| and \\left|Clock_{NTP_{Camera2}} - Clock_{GPS_{Camera2}}\\right|."
    );
  });
  ...
  it("re-serializes a table with mixed inline math, hard breaks, and code spans idempotently", () => {
    const { first, second } = roundTrip(
      [
        "| Condition | Facts |",
        "| --- | --- |",
        "| Excessive absolute difference between primary and secondary time references | The system checks whether <br>$\\left|Clock_{NTP_{Camera1}} - Clock_{GPS_{Camera1}}\\right|$<br> and $\\left|Clock_{NTP_{Camera2}} - Clock_{GPS_{Camera2}}\\right|$ remain below `Time_difference_treshold`. |",
        "",
      ].join("\n")
    );
